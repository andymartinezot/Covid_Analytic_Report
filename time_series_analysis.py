...
def plot_states_ar(states=['CABA'], days=7, mode=Mode.CASES, filename=None, end_date=None):
	COLUMN = 'residencia_provincia_nombre'
	df = load_relevant_data_ar(True, mode)
	plot_data_new(df, states, days, mode, COLUMN, filename, end_date)
...
def plot_data_new(df, places, days, mode, column, filename, end_date):
	n = len(places)
	colors = plt.cm.Oranges(np.linspace(0.35,0.65,n))
	...
	for index, place in enumerate(places):
		cumulative_data = df[df[column] == place]#Acumula los casos diarios por provicia 		start_column = cumulative_data.columns.get_loc("1/22/20")
		#counts = cumulative_data.iloc[:, start_column:].diff(axis=1) # Converts from total case count to daily case count
		x_values = list(cumulative_data.columns[-days-offset:len(cumulative_data.columns)-offset])
		#y_values = [int(cumulative_data[col]) for col in x_values]
		y_values = [int(cumulative_data[col]) for col in x_values]
...
if __name__ == '__main__':
	...
	plot_states_ar(filename="estados_argentina_test.png")