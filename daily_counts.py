...
def plot_daily_count_states_ar(states=['CABA'], day=TEST_DATE, mode=Mode.CASES, filename=None):
	COLUMN = 'residencia_provincia_nombre'
	df = load_relevant_data_ar(True, mode)
	#df = load_relevant_data_ar(True, mode).groupby(COLUMN).sum().reset_index()
	plot_data_new(df, states, day, mode, COLUMN, filename)
...
def plot_data_new(df, places, day, mode, column, filename):
	n = len(places)
	colors = plt.cm.Reds(np.linspace(0.35,0.65,n))

	values = []
	for index, place in enumerate(places): #https://www.geeksforgeeks.org/enumerate-in-python/
		cumulative_data = df[df[column] == place]#Acumula los casos diarios por provicia 
		start_column = cumulative_data.columns.get_loc("5/9/21")
		values.append(int(cumulative_data[day]))
...
if __name__ == '__main__':
	...
	plot_daily_count_states_ar(states=['CABA', 'Buenos Aires'],day="5/9/21")
